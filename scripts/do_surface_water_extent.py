...
class Swe():
    ...
    def runSwe (self, metafile=None, reflectance=None, dem=None,
        ...
        Description:
          ...
          information.  The scene-based SWE application is executed on the
          specified input files.  If a log file was specified, then the output
        ...
        Inputs:
          ...
          mgt, mlt1, mlt2, b4lt1, b4lt2, b5lt1, b5lt2, per_shade -
              these are threshold values used in the SWE algorithm
          logfile - name of the logfile for logging information; if None then
              the output will be written to stdout
          usebin - this specifies if the SWE executable resides in the $BIN
              directory; if None then the SWE executable is expected to be
              in the PATH

        Returns:
            ERROR - error running the SWE application
            SUCCESS - successful processing

        Notes:
          1. The script obtains the path of the metadata file and changes
             directory to that path for running the scene-based SWE code.  If
             the metafile directory is not writable, then this script exits
             with an error.
        ...
        parser.add_option ('--usebin', dest='usebin', default=False,
            action='store_true',
            help="use BIN environment variable to find the SWE executable")
        ...
        # should we expect the SWE executable to be in the PATH or in the BIN
        # directory?
        if usebin:
            ...
            # don't use a path to the SWE executable
            bin_dir = ''
            msg = "SWE executable is expected to be in the PATH"
            log (msg)
