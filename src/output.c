...
2/12/2012    Gail Schmidt     Original Development (based on input routines
                              from the LEDAPS lndsr application)
4/26/2013    Song Guo         Modified for surface water extent use
...
2/12/2012    Gail Schmidt     Original Development (based on input routines
                              from the LEDAPS lndsr application)
4/26/2013    Song Guo         Modifeid for surface water extent use
...
)
{
    ...
    for (ib = 0; ib < nband; ib++)
    {
        ...
        if (ib < 4)
        {
            if (put_attr_string (this->sds[ib].id, &attr, MASK_INDEX) != SUCCESS)
            {
                sprintf (errmsg, "Error writing attribute (scale factor)");
                error_handler (true, FUNC_NAME, errmsg);
                return (ERROR);
