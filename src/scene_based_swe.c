...
int main (int argc, char *argv[])
{
    ...
    char *reflectance_infile;    /* input TOA or Surface Reflectance filename */
    char *dem_infile;        /* input DEM filename */
    ...
    int16 *raw_swe;                    /* Raw surface water extent */
    int16 *shaded_revised_swe;  /* Shaded revised SWE */
    int16 *cloud_corrected_swe; /* Cloud corrected SWE */
    int16 *shaded_cloud_swe;    /* Shaded revised & cloud corrected SWE */
    float *percent_slope;     /* percent slope values between 0.0 and 100.0 */
    float *percent_shaded;    /* percent shaded values between 0.0 and 100.0 */
    uint8 *deep_shadow_mask;  /* deep shadow mask */
    int16 *scaled_slope;  /* percent slope values between 0 and 10000 */
    int16 *scaled_shaded; /* percent shaded values between 0 and 10000 */
    int16 *dem;           /* input DEM data (meters) */
    Input_t *input;       /* input structure for both the TOA reflectance
                                    and brightness temperature products */
    Space_def_t space_def;       /* spatial definition information */
    Output_t *output;     /* output structure and metadata */

    FILE *dem_fptr;              /* input scene-based DEM file pointer */
    FILE *raw_swe_fptr;          /* raw SWE file pointer */
    FILE *shaded_swe_fptr;       /* shaded revised SWE file pointer */
    FILE *cloud_swe_fptr;        /* cloud corrected SWE file pointer */
    FILE *shaded_cloud_swe_fptr; /* shaded revised & cloud corrected SWE 
                                    file pointer */
    FILE *dem_output_fptr;       /* DEM output file pointer */
    FILE *scaled_slope_fptr;     /* slope revised & cloud corrected SWE 
                                    file pointer */
    FILE *scaled_shaded_fptr;    /* shaded revised & cloud corrected SWE 
                                    file pointer */
