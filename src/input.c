...
)
{
    ...
    if (this->sr_sds_file_id == HDF_ERROR)
    {
        ...
        free (this);  
        sprintf (errmsg, "Error opening the input surface reflectance file: %s",
            lndsr_name);
        error_handler (true, FUNC_NAME, errmsg);
        return NULL;
    }
    ...
    if (use_toa)
    {
        ...
        if (this->refl_sds_file_id == HDF_ERROR)
        {
            ...
            free (this->refl_file_name);
            free (this);  
            sprintf (errmsg, "Error opening the input TOA reflectance "
                    "file: %s", lndcal_name);
            error_handler (true, FUNC_NAME, errmsg);
            return NULL;
        }
    ...
    for (ib = 0; ib < this->nqa_band; ib++) 
    {
        ...
        /* Check for any errors during processing of the above for loop and
...
)
{
    ...
    if (SDreaddata (this->qa_sds[iband].id, start, NULL, nval, buf) ==
        HDF_ERROR)
    {
        sprintf (errmsg, "Error reading %d lines from QA band 10"
...
)
{
    ...
    if (iline < 0 || iline >= this->nlines)
    {
        strcpy (errmsg, "Invalid line number for fmask band");
        error_handler (true, FUNC_NAME, errmsg);
        return ERROR;
    }
